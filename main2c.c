 ...
 * Copyright 2018 Damien Zammit <damien@zamaudio.com>
 *
 * AtomBIOS disassembler + data structure dumper
 *
 * main2c.c:
 * Disassembler + frontend (to C code).
 *
...
const char *addrtypes[] = {
    "my_radeon_reg(0x%04x)", "param[0x%02x]", "work[0x%02x]", "fb[0x%02x]", "data[0x%04x]", "0x%04x <D_ID unimplemented>",
...
const optab_t optable[256] = {
    ...
    { op_destsrc, "=", D_REG, 0, 0 }, { op_destsrc, "=", D_PS, 0, 0 },
    { op_destsrc, "=", D_WS, 0, 0 },  { op_destsrc, "=", D_FB, 0, 0 },
    { op_destsrc, "=", D_PLL, 0, 0 }, { op_destsrc, "=", D_MC, 0, 0 },
    { op_destsrc, "&=", D_REG, 0, 0 },  { op_destsrc, "&=", D_PS, 0, 0 },
    { op_destsrc, "&=", D_WS, 0, 0 },   { op_destsrc, "&=", D_FB, 0, 0 },
    { op_destsrc, "&=", D_PLL, 0, 0 },  { op_destsrc, "&=", D_MC, 0, 0 },
    { op_destsrc, "|=", D_REG, 0, 0 },   { op_destsrc, "|=", D_PS, 0, 0 },
    ...
    { op_1x8,     "my_mdelay", D_hex8, 0, 0 },
    { op_1x8,     "my_udelay", D_hex8, 0, 0 },
    { op_1x8,     "CALL_TABLE", D_hex8, 0, INDEX_COMMAND_TABLE },
    { op_1x8,     "<deprecated> REPEAT", D_hex8, 0, 0 },
    { op_dest,    "= 0", D_REG, 0, 0 }, { op_dest, "= 0", D_PS, 0, 0 },
    { op_dest,    "= 0", D_WS, 0, 0 },  { op_dest, "= 0", D_FB, 0, 0 },
    { op_dest,    "= 0", D_PLL, 0, 0 }, { op_dest, "= 0", D_MC, 0, 0 },
...
const char *align_source[] = {
    "",
    "BYTES_01", "BYTES_12", "BYTES_23",
    "BYTE_0", "BYTE_1", "BYTE_2", "BYTE_3"
};
const char *align_byte[] = { "BYTE_0", "BYTE_1", "BYTE_2", "BYTE_3" };
const char *align_word[] = { "BYTES_01", "BYTES_12", "BYTES_23", "__WTF__" };
const char *align_long[] = { "", "_WTF_", "_WTF_", "_WTF_" };
...
int op_switch (uint8_t *d, char *out) {
	...
	case 1:
	    out += sprintf (out, "        case 0x%02x:\n            return ", *t++);
	    break;
	case 2:
	    out += sprintf (out, "        case 0x%04x:\n            return ", *(uint16_t *)t);
	    t   += 2;
	    break;
	case 4:
	    out += sprintf (out, "        case 0x%08x:\n            return ", *(uint32_t *)t);
	    t   += 4;
	    break;
	}
	out += sprintf (out, "0x%04x;\n", *(uint16_t *)t);
	t   += 2;
    }
    t += 2;
    return t - d;
